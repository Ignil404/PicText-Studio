...
- 🎨 Галерея из 15 шаблонов с градиентными фонами, эмодзи-декорациями и стикерами
- ✏️ Canvas-редактор: текст, стикеры и фигуры редактируются в реальном времени
- 🖨️ Серверный рендер через Pillow + cairosvg (SVG data URI → PNG)
- 📜 История рендеров: гостевая (по session ID) + пользовательская (по JWT)
- 📎 Публичные ссылки на созданные изображения (`/shared/{id}`)
- 🖼️ Embed страницы для встраивания
- 🔐 Регистрация/вход через email + пароль, JWT access + refresh tokens
- 👤 Профиль с информацией об аккаунте и счётчиком рендеров
- 🖼️ Загрузка аватарок пользователей
- 📤 Share modal с кнопками Telegram, VK, WhatsApp, Twitter
- 🖼️ OG meta tags для preview в соцсетях
- 🔄 Guest → user migration: гостевые рендеры мигрируют при входе
- ⬛ Stickers и ⬜ Shapes в редакторе
...
- 📡 Webhooks: настройка URL для уведомлений о событиях
- 📖 API Docs: интерактивная документация API
